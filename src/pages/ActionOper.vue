			...
			<table class="table" cellpadding="0" cellspacing="0">
				<thead>
					<tr>
						...
						<th>Тип действия</th>
						<th>Тип операции действия</th>
						<th>Группа действия</th>
						<th>Действие</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="actionOper in actionOperList" :key="actionOper.action__id">
						<td>{{ actionOper.action_id.well_id.well_name }}</td>
						<td>{{ actionOper.hanger_type_id.hanger_type_name }}</td>
						<td>{{ actionOper.action_id.well_id.dept_id.dept_name }}</td>
						<td>{{ actionOper.action_id.well_id.dept_id.dept_type_id.dept_type_name }}</td>
						<td>{{ actionOper.action_id.action_date_begin }}</td>
						...
						<td>{{ actionOper.action_id.action_type_id.action_group_id.action_group_name }}</td>
						<td>
							<div class="td__btns">
